    ...
    if ($file['error'] === UPLOAD_ERR_OK) {
        ...
        if ($fileSize > 10 * 1024 * 1024) {
            ...
            if ($fileExt === 'txt') {
                ...
                if ($content === false) {
                    $message = 'Error reading TXT file. Please ensure the file is not corrupted.';
                    $messageType = 'error';
                }
            } else {
            ...
            if ($content !== false && $messageType !== 'error') {
                if (empty(trim($content))) {
                    $message = 'File appears to be empty or contains no readable text. Please check your file.';
                    $messageType = 'error';
                } else {
                    ...
                    @file_put_contents($debugDir . '/last_timetable_upload.txt', $content);

                    // Parse and preview only - don't save yet
                    try {
                        $previewData = parseTimetableFilePreview($content);
                        if ($previewData && $previewData['total_sessions'] > 0) {
                            $message = "Found {$previewData['total_sessions']} sessions. Review and confirm to save.";
                            $messageType = 'info';
                        } else {
                            $message = 'No sessions found in file. Check logs/last_timetable_upload.txt to ensure the PDF text was extracted properly, and verify it matches the required format.';
                            $messageType = 'error';
                        }
                    } catch (Exception $e) {
...
<div class="upload-card">
    <h2 style="display: flex; align-items: center; gap: 12px;">
        <svg viewBox="0 0 24 24" style="width: 24px; height: 24px; color: #dce3ff;" fill="none" stroke="currentColor"
            stroke-width="1.7">
    ...
    <p
        style="color: rgba(220,230,255,0.75); font-size: 14px; line-height: 1.6; margin-bottom: 24px; font-weight: 400; letter-spacing: -0.01em;">
        Our universal parser auto-detects SESSION and CELCAT formats. Drop the official timetable file here and we will
        populate programmes, modules, venues and sessions automatically.</p>
    <div class="tags" style="display: flex; gap: 10px; margin-bottom: 24px; flex-wrap: wrap;">
        <span class="tag"
            style="display:inline-flex; align-items:center; gap:8px; background: rgba(59, 130, 246, 0.15); border: 1px solid rgba(59, 130, 246, 0.3); padding: 8px 14px; border-radius: 10px; font-size: 12px; color: rgba(220,230,255,0.9); font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"
                style="width:14px;height:14px; color: #93c5fd;">
                <path d="M12 2v4" />
                <path d="M20 12h4" />
                <path d="M0 12h4" />
                <path d="M18.36 5.64 15.5 8.5" />
                <path d="M5.64 5.64 8.5 8.5" />
            </svg> Automatic format detection
        </span>
        <span class="tag"
            style="display:inline-flex; align-items:center; gap:8px; background: rgba(59, 130, 246, 0.15); border: 1px solid rgba(59, 130, 246, 0.3); padding: 8px 14px; border-radius: 10px; font-size: 12px; color: rgba(220,230,255,0.9); font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"
            ...
            </svg> Validates modules, lecturers and venues
        </span>
    </div>
    <form method="POST" enctype="multipart/form-data">
        <div class="upload-area"
            style="border: 2px dashed rgba(59, 130, 246, 0.4); border-radius: 16px; padding: 40px; text-align: center; background: rgba(59, 130, 246, 0.08); transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);">
            ...
            <p
                style="margin-bottom: 16px; color: rgba(220,230,255,0.85); font-size: 14px; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
            ...
            <label for="fileInput" class="upload-btn"
                style="background: rgba(59, 130, 246, 0.2); color: #dce3ff; border: 1px solid rgba(59, 130, 246, 0.4); padding: 12px 24px; border-radius: 10px; cursor: pointer; font-size: 14px; font-weight: 600; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); display: inline-block; margin-top: 8px;"
                onmouseover="this.style.background='rgba(59, 130, 246, 0.3)'; this.style.borderColor='rgba(59, 130, 246, 0.5)'"
                onmouseout="this.style.background='rgba(59, 130, 246, 0.2)'; this.style.borderColor='rgba(59, 130, 246, 0.4)'">Choose
                File</label>
            <div id="fileName"
                style="margin-top: 16px; color: #93c5fd; font-size: 13px; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
            </div>
        </div>
        <button type="submit" class="upload-btn"
            style="width: 100%; margin-top: 20px; background: rgba(59, 130, 246, 0.25); color: #dce3ff; border: 1px solid rgba(59, 130, 246, 0.4); padding: 14px 24px; border-radius: 12px; cursor: pointer; font-size: 15px; font-weight: 600; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);"
            onmouseover="this.style.background='rgba(59, 130, 246, 0.35)'; this.style.borderColor='rgba(59, 130, 246, 0.5)'"
            onmouseout="this.style.background='rgba(59, 130, 246, 0.25)'; this.style.borderColor='rgba(59, 130, 246, 0.4)'">Parse
            Timetable</button>
    </form>
    <div style="text-align: right; margin-top: 20px;">
        <a href="exam_pdf_parser.php" class="btn btn-secondary"
            style="font-size: 13px; padding: 10px 18px; display: inline-flex; align-items: center; gap: 6px;"><svg
                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.7"
                style="width:14px;height:14px; color: rgba(220,230,255,0.8);">
                <rect x="3" y="4" width="18" height="18" rx="2" />
                <path d="M16 2v4" />
                <path d="M8 2v4" />
                <path d="M3 10h18" />
            </svg>Need exam timetables?</a>
    </div>
</div>
...
<?php if ($message): ?>
    <div class="success-banner"
        ...
        <span aria-hidden="true"
            style="color: <?= $messageType === 'error' ? '#fca5a5' : ($messageType === 'info' ? '#93c5fd' : '#6ee7b7') ?>; flex-shrink: 0;">
        ...
        <span
            style="color: rgba(220,230,255,0.9); font-size: 14px; font-weight: 500;"><?= htmlspecialchars($message) ?></span>
    </div>
<?php endif; ?>
...
<?php if ($previewData && !$confirmed): ?>
    ...
    <div class="results-card"
        style="background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.10); border-radius: 24px; padding: 28px; backdrop-filter: blur(14px); box-shadow: 0 4px 20px rgba(0,0,0,0.45); font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;">
        <div class="results-header"
            style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px; flex-wrap: wrap; gap: 16px;">
            <div>
                <h3
                    style="color: #e8edff; font-size: 20px; font-weight: 700; letter-spacing: -0.2px; margin-bottom: 12px; display: flex; align-items: center; gap: 10px;">
                    <svg viewBox="0 0 24 24" style="width: 20px; height: 20px; color: #dce3ff;" fill="none"
            ...
            <span class="tag"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                    style="width:14px;height:14px;">
                    <path d="M12 2a10 10 0 1 0 10 10A10.011 10.011 0 0 0 12 2Z" />
                    <path d="M12 6v6l4 2" />
                </svg> SESSION format detected</span>
        </div>
        ...
        <div class="results-header">
            <div>
                ...
                <div class="info-hint">
                    ...
                    <div>Review the import summary below. <strong>Skipped</strong> sessions usually indicate duplicates or
                        missing metadata.</div>
                </div>
            </div>
            <span class="tag"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                    style="width:14px;height:14px;">
        ...
        <?php if (!empty($parsingResults['skipped'])): ?>
            <div>
                <div class="details-toggle" onclick="toggleSkippedDetails()">
                    <svg id="skipChevron" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
